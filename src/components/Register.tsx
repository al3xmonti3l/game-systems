...
export default function Register() {
  ...
  const [congratsPlayer, setCongratsPlayer] = useState<{ name: string; conTitle: string; zodTitle: string } | null>(null);

  useEffect(() => {
    if (showCongrats) {
      // Trigger fade-in on next frame
      const t = requestAnimationFrame(() => setCongratsVisible(true));
  ...
  async function handleRegister(e: FormEvent) {
    ...
    if (!gender) { setError('Please select your character gender.'); return; }
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/auth-register`,
        {
          ...
          body: JSON.stringify({ email, characterName, password, gender, birthMonth, birthDay, birthYear }),
        }
      );
      ...
      // Show congrats overlay then route to class-select
      setCongratsPlayer({
        name: json.player.character_name ?? json.player.username,
        conTitle: json.player.constellation_title ?? '',
        zodTitle: json.player.zodiac_title ?? '',
      });
      setShowCongrats(true);

      setTimeout(() => {
        setCongratsVisible(false);
        setTimeout(() => navigate('/class-select', { replace: true }), 600);
      }, 3200);
    } catch {
  ...
  return (
    <>
      <div className="register-bg">
        ...
        <div className="register-panel">
          ...
          <div className="tab-row">
            <button className={`tab-btn ${tab === 'register' ? 'tab-active' : ''}`}
              onClick={() => { setTab('register'); setError(''); }} type="button">
              New Player
            </button>
            <button className={`tab-btn ${tab === 'login' ? 'tab-active' : ''}`}
              onClick={() => { setTab('login'); setError(''); }} type="button">
              Returning Player
            </button>
          </div>
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegister} className="register-form">
              ...
              <div className="field-group">
                <label className="field-label">
                  <span className="label-tag">SYS</span>Player Designation
                </label>
                <input
                  type="email"
                  className="field-input"
              ...
              {/* Gender selection */}
              <div className="field-group">
                <label className="field-label"><span className="label-tag">CHR</span>Character Gender</label>
                <div className="gender-row">
                  ...
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                      ...
                      <path d="M18 2l4 4-4 4M22 2H16M20 6l-6 6" />
                    </svg>
                    Male
                  </button>
                  <button
                    type="button"
                    className={`gender-btn ${gender === 'Female' ? 'gender-active gender-female' : ''}`}
      ...
      {/* ── Congrats overlay ── */}
      {showCongrats && congratsPlayer && (
        <div className={`congrats-overlay ${congratsVisible ? 'congrats-visible' : ''}`}>
          <div className="congrats-stars" />
          <div className="congrats-content">
            <div className="congrats-glyph">
              <svg width="56" height="56" viewBox="0 0 56 56" fill="none">
                <circle cx="28" cy="28" r="26" stroke="#4fc3f7" strokeWidth="1.5" strokeDasharray="4 3" />
                <circle cx="28" cy="28" r="16" stroke="#4fc3f7" strokeWidth="0.8" opacity="0.4" />
                <circle cx="28" cy="28" r="5" fill="#4fc3f7" />
                <path d="M28 2v8M28 46v8M2 28h8M46 28h8" stroke="#4fc3f7" strokeWidth="1.2" />
              </svg>
            </div>
            <p className="congrats-handle">{congratsPlayer.name}</p>
            <h2 className="congrats-headline">Congratulations for Existing.</h2>
            <p className="congrats-sub">Your consciousness has been permanently woven into the fabric of LifeForge.</p>
            {congratsPlayer.conTitle && (
              <div className="congrats-titles">
                <span className="ct-pill">{congratsPlayer.conTitle}</span>
                {congratsPlayer.zodTitle && <span className="ct-pill ct-zod">{congratsPlayer.zodTitle}</span>}
              </div>
            )}
            <div className="congrats-bar">
              <div className="congrats-bar-fill" />
            </div>
          </div>
        </div>
      )}
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tab-row {
          ...
          background: rgba(4,14,30,0.6); border: 1px solid rgba(79,195,247,0.12);
          border-radius: 10px; padding: 3px; gap: 3px; margin-bottom: 24px;
        }
        .tab-btn {
        ...
        .tab-active { background: rgba(79,195,247,0.12); color: #4fc3f7 !important; box-shadow: inset 0 0 0 1px rgba(79,195,247,0.25); }
        ...
        .gender-btn {
          ...
          background: rgba(4,14,30,0.8); border: 1px solid rgba(79,195,247,0.2);
          border-radius: 8px; color: rgba(150,185,215,0.6); font-size: 13px; font-weight: 600;
          letter-spacing: 0.06em; padding: 10px 14px; cursor: pointer;
          transition: all 0.2s;
        }
        .gender-btn:hover { border-color: rgba(79,195,247,0.5); color: #e0f4ff; }
        .gender-active { border-color: #4fc3f7 !important; color: #4fc3f7 !important; background: rgba(79,195,247,0.1) !important; box-shadow: 0 0 12px rgba(79,195,247,0.15); }
        .gender-female.gender-active { border-color: #f48fb1 !important; color: #f48fb1 !important; background: rgba(244,143,177,0.1) !important; box-shadow: 0 0 12px rgba(244,143,177,0.15); }
        ...
        .congrats-overlay {
          ...
          background: radial-gradient(ellipse at 50% 30%, #061424 0%, #020810 60%, #000205 100%);
          display: flex; align-items: center; justify-content: center;
          opacity: 0; transition: opacity 0.7s ease;
          font-family: 'Segoe UI', system-ui, sans-serif;
        }
        .congrats-visible { opacity: 1; }

        .congrats-stars {
          position: absolute; inset: 0; pointer-events: none;
          background-image:
            radial-gradient(1px 1px at 8% 12%, rgba(255,255,255,0.7) 0%, transparent 100%),
            radial-gradient(1px 1px at 22% 58%, rgba(255,255,255,0.5) 0%, transparent 100%),
            radial-gradient(1px 1px at 38% 28%, rgba(255,255,255,0.6) 0%, transparent 100%),
            radial-gradient(1px 1px at 54% 75%, rgba(255,255,255,0.4) 0%, transparent 100%),
            ...
            radial-gradient(1px 1px at 91% 85%, rgba(255,255,255,0.6) 0%, transparent 100%),
            radial-gradient(2px 2px at 48% 38%, rgba(79,195,247,0.3) 0%, transparent 100%),
            radial-gradient(2px 2px at 28% 72%, rgba(79,195,247,0.25) 0%, transparent 100%);
        }

        .congrats-content {
          position: relative; z-index: 1; text-align: center;
          display: flex; flex-direction: column; align-items: center; gap: 16px;
          max-width: 500px; padding: 0 24px;
          animation: congratsRise 0.8s ease forwards;
        }
        ...
        .congrats-glyph { animation: rotateSlow 12s linear infinite; }
        ...
        .congrats-headline {
          ...
          color: #e8f4ff;
          text-shadow: 0 0 40px rgba(79,195,247,0.5), 0 0 80px rgba(79,195,247,0.2);
        }

        .congrats-sub {
          font-size: 13px; line-height: 1.7; color: rgba(140,180,220,0.55); max-width: 380px;
        }

        .congrats-titles { display: flex; gap: 10px; flex-wrap: wrap; justify-content: center; }
        .ct-pill {
          font-size: 11px; font-weight: 600; letter-spacing: 0.1em;
          background: rgba(79,195,247,0.1); border: 1px solid rgba(79,195,247,0.3);
          color: #4fc3f7; padding: 4px 12px; border-radius: 6px;
        }
        .ct-zod {
          background: rgba(243,156,18,0.1); border-color: rgba(243,156,18,0.3); color: #f39c12;
        }

        .congrats-bar {
          width: 240px; height: 2px; background: rgba(79,195,247,0.1);
        ...
        @media (max-width: 520px) { .register-panel { padding: 28px 24px 24px; } .panel-title { font-size: 22px; } .congrats-headline { font-size: 24px; } }
      `}</style>
    </>
  );
}
